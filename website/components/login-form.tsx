...
export function LoginForm({
  ...
  const mutation = useMutation({
    mutationFn: async (credentials: { username: string; password: string }) => {
      const response = await fetch(venv.SERVER + "/auth/authenticate", {
        method: "POST",
        credentials: 'include',
        headers: {
          ...
          <form className="p-6 md:p-8" onSubmit={login}>
            <div className="flex flex-col gap-6 h-[400px] justify-center">
              ...
              <div className="grid gap-3">
                <Label htmlFor="username">{venv.LOGIN_FIELD_LABEL}</Label>
                <Input
                  id="username"
                  name="username"
                  placeholder={venv.LOGIN_FIELD_LABEL === "Student Number" ? "30211935" : venv.LOGIN_FIELD_LABEL}
                  required
                />
              </div>
              <div className="grid gap-3">

...
export default function CreateServerWizard() {
    ...
    const abbrev_name = function (str1: string) {
        if (!str1) return auth?.username || "Unknown";
    ...
    const mutation = useMutation({
        mutationFn: async (vmConfig: {
            ...
            const response = await fetch(venv.SERVER + `/vm/${team}`, {
                ...
                body: JSON.stringify({
                    name: (vmConfig.course_name || "").split(" ").join("-") + " "
                    + (auth.username || "").split(" ").join("-") + " "
                    + abbrev_name(auth.displayName).split(" ").join("-") + " "
                    + vmConfig.name,
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const vmConfig = {
            ...
            isoPath: selectedIso || undefined,
            networkSwitch: selectedNetwork || undefined
        }
    ...
    return (
        <SidebarMenu>
            <SidebarMenuItem>
                <Drawer direction="right" open={open} onOpenChange={(v) => { setOpen(v); if (v) { setShowError(false); setErrorMessage(""); } }}>
                                        ...
                                        showError == true &&
                                            <div className="w-full flex justify-center">
                                                <Alert variant="destructive" className="w-[90%]">
                                                    ...
                                                    <AlertDescription>
                                                    {errorMessage && <p className="font-mono text-xs mb-2">{errorMessage}</p>}
                                                    <p>Could be the following issues.</p>
                                                    <ul className="list-inside list-disc text-sm">
                                                        <li>You tried to assign more than your quota</li>
                                                        <li>There was an internal server error</li>
                                                        <li>One of the values is unset</li>
                                                    </ul>
                                                    <p>If this does not help, please contact your lecturer</p>
                                                    </AlertDescription>
                                                </Alert>                                            
                                            </div>
                                    }
                                    <form className="min-h-full" onSubmit={handleSubmit}>
                                        <div className="p-10 pt-5 flex-col flex gap-y-5">
                                            ...
                                            <div>
                                                ...
                                                <Select name="os" value={selectedIso} onValueChange={setSelectedIso} required>
                                            ...
                                            <div>
                                                ...
                                                <Select name="network_switch" value={selectedNetwork} onValueChange={setSelectedNetwork} required>
                                                    <SelectTrigger className="w-full">
